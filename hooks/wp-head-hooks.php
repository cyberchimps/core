		...
		<style type="text/css" media="all">
			...
			body {
				...
				foreach ( $body_styles as $key => $body_style ) :
					?>
					<?php echo esc_attr( $key ); ?> : <?php echo esc_html( $body_style ); ?>;
			<?php endforeach; ?>
			}
			...
			<?php if ( ! empty( $link_styles ) ) : ?>
				<?php foreach ( $link_styles as $key2 => $link_style ) : ?>
					...
					{
						color: <?php echo esc_html( $link_style ); ?>;
					}
				<?php endforeach; ?>
			<?php endif; ?>
			...
			.container {
				...
				foreach ( $container_styles as $key3 => $container_style ) :
					?>
					<?php echo esc_html( $key3 ); ?> : <?php echo esc_html( $container_style ); ?>px;
			<?php endforeach; ?>
			}
			...
			<?php if ( ! empty( $headings_styles[0] ) ) { ?>
					h1 {
						...
						foreach ( $headings_styles[0] as $key => $headings_style_1 ) {
							// Changed to previous code for minor font changes.
							$headings_style_1 = empty( $headings_style_1 ) ? '' : $headings_style_1;
							echo esc_html( $key );
							?>
							: <?php echo esc_html( $headings_style_1 ); ?>;
						<?php } ?>
						}
				<?php
}
	...
	?>
					h2 {
						...
						foreach ( $headings_styles[1] as $key => $headings_style_2 ) {
							$headings_style_2 = empty( $headings_style_2 ) ? '' : $headings_style_2;
							echo esc_html( $key );
							?>
							: <?php echo esc_html( $headings_style_2 ); ?>;
						<?php } ?>
						}
			<?php
}
	...
	?>
					h3 {
						...
						foreach ( $headings_styles[2] as $key => $headings_style_3 ) {
							$headings_style_3 = empty( $headings_style_3 ) ? '' : $headings_style_3;
							echo esc_html( $key );
							?>
							: <?php echo esc_html( $headings_style_3 ); ?>;
						<?php } ?>
						}
			<?php
}
...
if ( ! cyberchimps_get_option( 'responsive_design', 'checked' ) ) {
	?>
				@media screen and (max-width: <?php echo esc_html( $width ); ?>) {
...
function cyberchimps_headings_styles() {
	...
	if ( 'Google Fonts' === $headings_styles_1['font-family'] && '' !== $google_font_h1 ) {
		$headings_styles_1['font-family'] = '"' . $google_font_h1 . '"';

		// Check if SSL is present, if so then use https othereise use http.
		$protocol = is_ssl() ? 'https' : 'http';

		wp_register_style( 'google-font-h1', $protocol . '://fonts.googleapis.com/css?family=' . $google_font_h1, array(), '1.1' );
		wp_enqueue_style( 'google-font-h1' );
	}

	// Check if Google fonts have been selected - h2.
	if ( 'Google Fonts' === $headings_styles_2['font-family'] && '' !== $google_font_h2 ) {
		$headings_styles_2['font-family'] = '"' . $google_font_h2 . '"';

		// Check if SSL is present, if so then use https othereise use http.
		$protocol = is_ssl() ? 'https' : 'http';
		wp_register_style( 'google-font-h2', $protocol . '://fonts.googleapis.com/css?family=' . $google_font_h2, array(), '1.1' );
		wp_enqueue_style( 'google-font-h2' );
	}

	// Check if Google fonts have been selected - h3.
	if ( 'Google Fonts' === $headings_styles_3['font-family'] && '' !== $google_font_h3 ) {
		$headings_styles_3['font-family'] = '"' . $google_font_h3 . '"';

		// Check if SSL is present, if so then use https othereise use http.
		$protocol = is_ssl() ? 'https' : 'http';

		wp_register_style( 'google-font-h3', $protocol . '://fonts.googleapis.com/css?family=' . $google_font_h3, array(), '1.1' );
...
function cyberchimps_body_styles() {
	...
	if ( 'Google Fonts' === $body_styles['font-family'] && '' !== $google_font ) {
		...
		wp_register_style( 'google-font', $protocol . '://fonts.googleapis.com/css?family=' . $google_font, array(), '1.1' );
...
function cyberchimps_favicon() {
	if ( cyberchimps_get_option( 'custom_favicon' ) ) :
		...
		if ( '' !== $favicon ) :
			?>
			<link rel="shortcut icon" href="<?php echo esc_url( stripslashes( $favicon ) ); ?>" type="image/x-icon"/>
			<?php
		endif;
	endif;
}
