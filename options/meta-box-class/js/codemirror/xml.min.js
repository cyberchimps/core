CodeMirror.defineMode("xml", function (w, k) {
	var p = w.indentUnit;
	...
	function i(y, A) {
		var z = v.doNotIndent.hasOwnProperty(y) || (l.context && l.context.noIndent);
	...
	function c(y) {
		return function (z) {
			if (z == "selfcloseTag" || (z == "endTag" && v.autoSelfClosers.hasOwnProperty(l.tagName.toLowerCase()))) {
				return e()
			}
			if (z == "endTag") {
				i(l.tagName, y);
	...
	return{startState: function () {
		...
		A.startOfLine = false;
		return h || z
	}, indent: function (B, y, A) {
		var z = B.context;
		if ((B.tokenize != n && B.tokenize != o) || z && z.noIndent) {
	...
	}, compareStates: function (B, z) {
		if (B.indented != z.indented || B.tokenize != z.tokenize) {
			return false
		}
		for (var A = B.context, y = z.context; ; A = A.prev, y = y.prev) {
			if (!A || !y) {
				return A == y
			}
			if (A.tagName != y.tagName) {
				return false
			}
		}
	}, electricChars: "/"}
